...
impl Store {
    pub fn open(data_dir: &Path) -> Result<Arc<Self>, String> {
        ...
        connection.execute_batch(
            "PRAGMA journal_mode = WAL;
             ...
             CREATE TABLE IF NOT EXISTS app_rules (executable TEXT PRIMARY KEY, display_name TEXT NOT NULL, process_path TEXT NOT NULL, category_id INTEGER, ignored INTEGER NOT NULL DEFAULT 0, FOREIGN KEY(category_id) REFERENCES categories(id));
    ...
    pub fn rule_for(
        ...
        connection.query_row(
            "SELECT r.display_name, r.process_path, r.ignored, COALESCE(c.name, '其他') FROM app_rules r LEFT JOIN categories c ON c.id = r.category_id WHERE r.executable = ?1",
            params![executable], |row| Ok(AppInfo { executable: executable.into(), display_name: row.get(0)?, process_path: row.get(1)?, ignored: row.get::<_, i64>(2)? != 0, category: row.get(3)? })
        ).map_err(|err| err.to_string())
    }
    ...
    pub fn rules(&self) -> Result<Vec<AppRule>, String> {
        let connection = self.connection.lock();
        let mut statement = connection.prepare("SELECT r.executable, r.display_name, r.category_id, COALESCE(c.name, '其他'), r.ignored FROM app_rules r LEFT JOIN categories c ON c.id=r.category_id ORDER BY r.display_name COLLATE NOCASE").map_err(|err| err.to_string())?;
        let rules = statement
            .query_map([], |row| {
                Ok(AppRule {
    ...
    pub fn update_rule(&self, rule: &AppRule) -> Result<(), String> {
        let connection = self.connection.lock();
        connection.execute("UPDATE app_rules SET display_name=?1, category_id=?2, ignored=?3 WHERE executable=?4", params![rule.display_name.trim(), rule.category_id, rule.ignored as i64, rule.executable]).map(|_| ()).map_err(|err| err.to_string())
    }
    ...
    pub fn week_summary(&self, date: &str) -> Result<WeekSummary, String> {
        let selected =
            NaiveDate::parse_from_str(date, "%Y-%m-%d").map_err(|err| err.to_string())?;
        let week_start = selected
            .checked_sub_days(Days::new(selected.weekday().num_days_from_monday() as u64))
            .ok_or("Date is out of range")?;
        let week_end = week_start
            .checked_add_days(Days::new(7))
            .ok_or("Date is out of range")?;
        let (start, _) = Self::date_range(week_start)?;
        let (end, _) = Self::date_range(week_end)?;
        let days = self.bucket_usage(
            (0..7).filter_map(|offset| week_start.checked_add_days(Days::new(offset))),
            "%m/%d",
        )?;
    ...
    pub fn month_summary(&self, date: &str) -> Result<MonthSummary, String> {
        let selected =
            NaiveDate::parse_from_str(date, "%Y-%m-%d").map_err(|err| err.to_string())?;
        let month_start =
            NaiveDate::from_ymd_opt(selected.year(), selected.month(), 1).ok_or("Invalid month")?;
        ...
        .ok_or("Unable to determine next month")?;
        let (start, _) = Self::date_range(month_start)?;
        let (end, _) = Self::date_range(next_month)?;
        let days = self.bucket_usage(
            std::iter::successors(Some(month_start), |day| day.checked_add_days(Days::new(1)))
                .take_while(|day| *day < next_month),
            "%d",
        )?;
        let categories = self.grouped("SELECT COALESCE(c.name, '其他'), SUM(MAX(0, MIN(s.ended_at, ?2) - MAX(s.started_at, ?1)) / 1000), COALESCE(c.color, '#8391A5') FROM sessions s LEFT JOIN app_rules r ON r.executable=s.executable LEFT JOIN categories c ON c.id=r.category_id WHERE s.ended_at>?1 AND s.started_at<?2 GROUP BY c.name,c.color ORDER BY 2 DESC", &[&start, &end])?;
        let applications = self.application_usage("SELECT COALESCE(r.display_name, s.display_name), SUM(MAX(0, MIN(s.ended_at, ?2) - MAX(s.started_at, ?1)) / 1000), s.executable, MAX(s.process_path) FROM sessions s LEFT JOIN app_rules r ON r.executable=s.executable WHERE s.ended_at>?1 AND s.started_at<?2 GROUP BY s.executable, r.display_name ORDER BY 2 DESC LIMIT 10", &[&start, &end])?;
    ...
    pub fn all_summary(&self) -> Result<AllSummary, String> {
        ...
        let categories = self.grouped("SELECT COALESCE(c.name, '其他'), SUM((s.ended_at-s.started_at)/1000), COALESCE(c.color, '#8391A5') FROM sessions s LEFT JOIN app_rules r ON r.executable=s.executable LEFT JOIN categories c ON c.id=r.category_id GROUP BY c.name,c.color ORDER BY 2 DESC", &[])?;
        let applications = self.application_usage("SELECT COALESCE(r.display_name, s.display_name), SUM((s.ended_at-s.started_at)/1000), s.executable, MAX(s.process_path) FROM sessions s LEFT JOIN app_rules r ON r.executable=s.executable GROUP BY s.executable, r.display_name ORDER BY 2 DESC LIMIT 10", &[])?;
        Ok(AllSummary {
            months,
            categories,
            applications,
...
mod tests {
    ...
    #[test]
    fn week_summary_uses_monday_and_clips_crossing_sessions() {
        let (_directory, store) = test_store();
        insert_session(
            &store,
        ...
        let summary = store.week_summary("2026-07-15").unwrap();
    ...
    #[test]
    fn month_summary_rejects_malformed_dates() {
        let (_directory, store) = test_store();
        assert!(store.month_summary("x").is_err());
        assert!(store.month_summary("2026-99-01").is_err());
    }
